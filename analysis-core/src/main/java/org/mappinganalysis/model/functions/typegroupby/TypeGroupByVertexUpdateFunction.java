...
//    LOG.info("potentiallyUpdatingVertex: " + vertex.toString());
    if (!vertex.getValue().containsKey(Constants.TMP_TYPE) && vertex.getValue().hasTypeNoType(Constants.COMP_TYPE)) {
...
//      LOG.info("vertex " + vertex.getId() + " got message from: " + msg.get(Utils.VERTEX_ID) + " " + msg.getHashCcId());
      ...
      if (vertexCcId != msgCcId) {
        double newSim = (double) msg.get(Constants.AGGREGATED_SIM_VALUE);
//        options.put((long) msg.get(Utils.VERTEX_ID), newSim);

        boolean isSpecialCondition = newBestValue.hasTypeNoType(Constants.COMP_TYPE)
...
//        }
        ...
        if (Doubles.compare(newSim, bestSim) > 0 || isSpecialCondition) {
          bestSim = newSim;
          if (msg.containsKey(Constants.TMP_TYPE) || !msg.hasTypeNoType(Constants.COMP_TYPE)) {
//            LOG.info("tmp type or has type: " + msg);
            newBestValue = msg;
          } else if (msg.hasTypeNoType(Constants.COMP_TYPE)) {
...
//            LOG.info("nbv has no type: " + newBestValue.toString());
        ...
        } else if (msg.containsKey(Constants.TMP_TYPE) || !msg.hasTypeNoType(Constants.COMP_TYPE)
...
//        addNeighborOptions(vertex, options, msg);
  ...
  private void updateVertexValue(Vertex<Long, ObjectMap> vertex,
    ...
    vertex.getValue().put(Constants.HASH_CC, newBestValue.getHashCcId());
//    vertex.getValue().put(Utils.VERTEX_OPTIONS, options);
    if (newBestValue.containsKey(Constants.TMP_TYPE)) {
      vertex.getValue().put(Constants.TMP_TYPE, newBestValue.get(Constants.TMP_TYPE));
    }
    if (newBestValue.containsKey(Constants.COMP_TYPE) && !newBestValue.hasTypeNoType(Constants.COMP_TYPE)) {
      vertex.getValue().put(Constants.TMP_TYPE, newBestValue.get(Constants.COMP_TYPE));
    }
  }

  private HashMap<Long, Double> initOptions(Vertex<Long, ObjectMap> vertex) {
    HashMap<Long, Double> options;
    if (vertex.getValue().containsKey(Constants.VERTEX_OPTIONS)) {
      options = (HashMap<Long, Double>) vertex.getValue().get(Constants.VERTEX_OPTIONS);
    } else {
  ...
  private void addNeighborOptions(Vertex<Long, ObjectMap> vertex, HashMap<Long, Double> options, ObjectMap msg) {
    if (msg.containsKey(Constants.VERTEX_OPTIONS)) {
      HashMap<Long, Double> tmp = (HashMap<Long, Double>) msg.get(Constants.VERTEX_OPTIONS);
    ...
    options.remove(msg.get(Constants.VERTEX_ID));
  }
}
